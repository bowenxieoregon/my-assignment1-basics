...
def run_linear(
    ...
    linear_layer.load_state_dict({"weight": weights}, strict = False)
...
def run_embedding(
    ...
    embedding_layer.load_state_dict({"weight": weights}, strict = False)
...
def run_swiglu(
    ...
    swiglu = positionwise_feedforward(d_ff=d_ff, d_model=d_model)
    swiglu.load_state_dict({"w1.weight":w1_weight, "w2.weight": w2_weight, "w3.weight": w3_weight}, strict=False)
...
def run_multihead_self_attention(
    ...
    MHA.load_state_dict({"q_proj.weight": q_proj_weight, "k_proj.weight": k_proj_weight, "v_proj.weight": v_proj_weight, "output_proj.weight": o_proj_weight}, strict=False)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    from cs336_basics.model import multihead_self_attention, ROPE
    rope = ROPE(theta=theta, d_k = d_model // num_heads, max_seq_len=max_seq_len)
    MHA = multihead_self_attention(d_model=d_model, num_heads=num_heads, pos_embedding=rope)
    MHA.load_state_dict({"q_proj.weight": q_proj_weight, "k_proj.weight": k_proj_weight, "v_proj.weight": v_proj_weight, "output_proj.weight": o_proj_weight}, strict=False)
...
def run_transformer_block(
    ...
    """
    from cs336_basics.model import transformer_block, ROPE
    rope = ROPE(theta=theta, d_k= d_model // num_heads, max_seq_len=max_seq_len)
    block = transformer_block(d_model = d_model, num_heads = num_heads, d_ff = d_ff, pos_embedding = rope)
...
def run_transformer_lm(
    ...
    """
    from cs336_basics.model import transformer_lm, ROPE
    rope = ROPE(theta=rope_theta, d_k= d_model // num_heads, max_seq_len=context_length)
    lm = transformer_lm(vocab_size=vocab_size, d_model=d_model, num_heads=num_heads, d_ff=d_ff, context_length=context_length, num_layers = num_layers, pos_embedding = rope)
...
def run_rmsnorm(
    ...
    rms_norm.load_state_dict({"weight": weights},strict=False)
...
def run_save_checkpoint(
    ...
    """
    from cs336_basics.checkpointing import save_checkpointing
    save_checkpointing(model=model, optimizer=optimizer, iteration=iteration, out=out)
...
def run_load_checkpoint(
    ...
    """
    from cs336_basics.checkpointing import load_checkpointing
    it = load_checkpointing(src=src, model=model, optimizer=optimizer)
    return it
